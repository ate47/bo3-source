...
function function_646cd830(a_ents) {
    /#
        recordent(a_ents["street_balcony_spawn_closet_available"]);
    #/
    /#
        recordent(a_ents["flag_start_plyr_controlling_vo"]);
    #/
...
function function_95c63963() {
    self endon(#"death");
    assert(isdefined(self.script_string), "garage_robots_spawn_manager_trig");
...
/#
    ...
    function function_9075d8d6() {
        ...
        for (n_stage = 0; true; n_stage++) {
            self thread namespace_8e9083ff::function_ff016910("street_allies_attack_colortrig" + n_stage, undefined);
