...
function function_f8b6e626(name) {
    ...
    for (var_c506f9a7 = 0; var_c506f9a7 < teamops.modes.size; var_c506f9a7++) {
        if (teamops.modes[var_c506f9a7] == level.gametype) {
            return true;
        }
    }
    return false;
...
function function_a2630ab8(name) {
    ...
    game["teamops"].var_b7b1a955 = randomintrange(0, teamops.rewards.size);
    game["teamops"].var_f5dde2d3 = teamops.rewards[game["teamops"].var_b7b1a955];
    game["teamops"].var_dfb9b8a5 = gettime();
    foreach (team in level.teams) {
        game["teamops"].teamprogress[team] = 0;
    }
    wait 0.1;
    function_cf51fcaa(game["teamops"].var_cdbaab59, game["teamops"].var_b7b1a955, game["teamops"].var_dfb9b8a5, teamops.time);
    wait 0.1;
    function_48a94128(1);
...
function function_f0900ae5(event, player, team) {
    ...
    if (isdefined(teamops.var_95a7033b) && event == teamops.var_95a7033b) {
        game["teamops"].teamprogress[team] = game["teamops"].teamprogress[team] + 1;
        level function_3d345413(event, player, team);
    }
    if (isdefined(teamops.var_3795699a) && event == teamops.var_3795699a) {
        game["teamops"].teamprogress[team] = game["teamops"].teamprogress[team] - 1;
    ...
    if (isdefined(teamops.var_3e8aa3fe) && event == teamops.var_3e8aa3fe) {
        game["teamops"].teamprogress[team] = 0;
        level function_3d345413(event, player, team);
    }
}
