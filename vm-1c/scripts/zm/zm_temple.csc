...
function function_98ca8bd1(localclientnum, oldval, newval, bnewent, binitialsnap, fieldname, bwasdemojump) {
    zm_temple_maze::function_469b0955(localclientnum, newval);
}
...
/#
    ...
    function function_53be9070() {
        ...
        for (i = 0; i < 5; i++) {
            print3d(self.origin + offset, "<dev string:x63>", (60, 60, -1), 1, scale, 10);
            scale *= 1.7;
            offset += dir * 6;
        }
    }
...
function function_455a41fb(clientnum, num) {
    ...
    for (i = 3; i >= 0; i--) {
        ...
        if (num >= pow) {
            num -= pow;
            println("<dev string:x118>" + clientnum + "<dev string:x128>" + i + "<dev string:x132>" + level.var_1dfcee77[clientnum][i].size);
            array::thread_all(level.var_1dfcee77[clientnum][i], &function_a445d032);
            continue;
        }
        println("<dev string:x134>" + clientnum + "<dev string:x128>" + i + "<dev string:x132>" + level.var_1dfcee77[clientnum][i].size);
...
function function_2462b329(var_6870ff1c) {
    ...
    println("<dev string:x143>" + var_6870ff1c + "<dev string:x14e>");
    for (i = 0; i < level.var_1dfcee77[var_6870ff1c].size; i++) {
        var_1dfcee77 = getentarray(var_6870ff1c, "pap_spinner" + i + 1, "targetname");
        println("<dev string:x143>" + var_6870ff1c + "<dev string:x128>" + i + "<dev string:x132>" + var_1dfcee77.size);
