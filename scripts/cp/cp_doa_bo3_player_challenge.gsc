...
function function_31c377e( room )
{
    ...
    room.glow = [];
    glow = spawn( "script_model", doa_arena::function_61d60e0b() + ( 0, 0, 36 ) );
    ...
    glow = spawn( "script_model", doa_arena::function_61d60e0b() + ( 0, 0, 72 ) );
    glow.targetname = "spiralglow2";
    glow setmodel( "tag_origin" );
    glow thread doa_fx::function_285a2999( "glow_blue" );
    room.glow[ room.glow.size ] = glow;
    glow = spawn( "script_model", doa_arena::function_61d60e0b() + ( 0, 0, 128 ) );
    glow.targetname = "spiralglow3";
    glow setmodel( "tag_origin" );
    glow thread doa_fx::function_285a2999( "glow_blue" );
    room.glow[ room.glow.size ] = glow;
    glow = spawn( "script_model", doa_arena::function_61d60e0b() + ( 0, 0, 160 ) );
...
function function_fe1ce5f1( einflictor, eattacker, idamage, idflags, smeansofdeath, weapon, vpoint, vdir, shitloc, vdamageorigin, psoffsettime, damagefromunderneath, modelindex, partname )
{
    ...
    while ( var_516eed4b && player.room.var_e01f23f0.size < 275 )
    {
        ...
        reward = "zombietron_beryl";
        player.room.var_e01f23f0[ player.room.var_e01f23f0.size ] = level doa_pickups::spawnubertreasure( self.origin + ( 0, 0, 48 ), 1, 70, 1, 1, 2, reward, 0.2, 1, 0, 0 )[ 0 ];
        var_516eed4b--;
    }
...
function function_c2b99e74( room )
{
    ...
    foreach ( player in getplayers() )
    {
        ...
        {
            /#
                doa_utility::debugmsg( "<dev string:x45>" + player.name );
            #/
    ...
    foreach ( player in getplayers() )
    {
        ...
        {
            /#
                doa_utility::debugmsg( "<dev string:x5d>" + player.name );
            #/
...
function function_60fcd122( room, goaltrigger )
{
    ...
    for ( next = struct::get( goaltrigger.posts[ 0 ].target, "targetname" ); next != goaltrigger.posts[ 0 ] ; next = struct::get( next.target, "targetname" ) )
    {
...
function function_dfbad276( number, startside )
{
    ...
    while ( number > 0 )
    {
        ...
        trace = bullettrace( dest_point.origin, dest_point.origin + ( 0, 0, -500 ), 0, undefined );
    ...
    while ( true )
    {
        ...
        if ( dmg_type == "MOD_PROJECTILE" || dmg_type == "MOD_GRENADE" || dmg_type == "MOD_CRUSH" || weapon == level.doa.var_69899304 )
        {
            ...
            if ( isdefined( self.sacred ) )
            {
                if ( isplayer( attacker ) )
                {
                    attacker.doa.goldenCowsExploded++;
                }
