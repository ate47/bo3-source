...
function function_11600557( zombie, player )
{
    ...
    if ( player.var_5206d4b9 >= level.var_a9e78bf7[ "maxreachabilitylevel" ] )
    {
        ...
        {
            rocketstart = origin + ( 0, 0, 80 );
            
            if ( distancesquared( player.origin, origin ) > 40000 )
            {
                rocketend = player.origin + ( 0, 0, 35 );
                var_b990e3ba = getactorteamarray( "axis" );
                
                if ( isdefined( var_b990e3ba ) && var_b990e3ba.size )
                {
                    rocket = magicbullet( level.var_9e70dea1, rocketstart, rocketend, undefined, array::random( var_b990e3ba ) );
...
{
    self endon( #"death" );
    level waittill( #"BZM_kill_active_parasites_meatball_rocket" );
    self delete();
}
...
function function_2ffffb72( entity )
{
    ...
    assert( entity.archetype == "<dev string:x37>" );
...
function function_abc05be8()
{
    ...
    /#
        recordenttext( "<dev string:x3e>", self, ( 0, 1, 0 ), "<dev string:x4e>" );
    #/
...
function function_55e64fc7()
{
    ...
    while ( true )
    {
        ...
        {
            /#
                recordenttext( "<dev string:x55>", self, ( 1, 0, 0 ), "<dev string:x4e>" );
            #/
...
function function_d94c729a()
{
    ...
    if ( isdefined( self.var_be85d412 ) && self.var_be85d412 )
    {
        ...
        /#
            recordenttext( "<dev string:x5f>", self, ( 0, 1, 0 ), "<dev string:x4e>" );
        #/
    }
}
            ...
            foreach ( zombie in var_b990e3ba )
            {
                if ( isdefined( zombie ) && zombie.archetype == "zombie" && zombie haspath() )
                {
                    ...
                    if ( var_a6c6348c.size )
                    {
                        ...
                        /#
                            recordline( self.origin, teleportorigin, ( 1, 0, 0 ), "<dev string:x4e>" );
                        #/
...
function function_48401071()
{
    ...
    while ( true )
    {
        if ( isdefined( self.var_506b9d18 ) && ( isdefined( self.in_the_ground ) && self.in_the_ground || self.var_506b9d18 ) )
        {
            ...
            /#
                recordenttext( "<dev string:x6d>", self, ( 0, 1, 0 ), "<dev string:x4e>" );
            #/
        ...
        if ( !var_382bd177 && !var_6fc198b2 )
        {
            ...
            /#
                recordenttext( "<dev string:x87>", self, ( 0, 1, 0 ), "<dev string:x4e>" );
            #/
        ...
        if ( self isinscriptedstate() )
        {
            ...
            {
                /#
                    recordenttext( "<dev string:xa1>", self, ( 0, 1, 0 ), "<dev string:x4e>" );
                #/
        ...
        {
            /#
                recordenttext( "<dev string:xd1>", self, ( 0, 1, 0 ), "<dev string:x4e>" );
            #/
...
{
    assert( self.archetype == "<dev string:x37>" );
...
function function_7effc669( var_f85c1f6a )
{
    ...
    if ( bonuszmutil::function_5f2c4513() <= 0 )
    {
        return false;
    }
...
{
    if ( level.var_b1955bd6 >= bonuszmutil::function_5f2c4513() )
    {
        return;
    }
    ...
    if ( isdefined( var_28b84d73 ) )
    {
        ...
        /#
            ...
            {
                recordcircle( var_c3afd9e9, 8, ( 1, 0.5, 0 ), "<dev string:x4e>" );
                recordline( var_c3afd9e9, level.activeplayers[ 0 ].origin, ( 1, 0.5, 0 ), "<dev string:x4e>" );
                record3dtext( "<dev string:xf4>", level.activeplayers[ 0 ].origin, ( 1, 0.5, 0 ), "<dev string:x4e>" );
                iprintln( "<dev string:x109>" + gettime() );
            }
        #/
...
function function_cc657250()
{
    ...
    if ( level.var_d0e37460 >= bonuszmutil::function_5f2c4513() )
    {
...
function zombie_death_event( zombie )
{
    ...
    if ( isdefined( zombie.var_b7a92141 ) && zombie.var_b7a92141 )
    {
        ...
        {
            radiusdamage( zombie.origin + ( 0, 0, 35 ), 200, 70, 20, undefined, "MOD_EXPLOSIVE" );
        }
...
function bzmutil_waitforallzombiestodie()
{
    ...
    while ( true )
    {
        if ( isdefined( level.var_a9e78bf7 ) && level.var_a9e78bf7[ "zombifyenabled" ] )
        {
            if ( !level.var_b1955bd6 && bonuszmutil::function_5f2c4513() || !level flag::get( "bzmExtraZombieCleared" ) )
            {
                wait 1;
                continue;
            }
...
// Size: 0x172
function function_88adb698( skiptoname, levelstart )
{
    if ( !sessionmodeiscampaignzombiesgame() )
    {
        return;
    ...
    if ( !bonuszmutil::function_5f2c4513() )
    {
        level flag::set( "bzmExtraZombieCleared" );
    }
    else
