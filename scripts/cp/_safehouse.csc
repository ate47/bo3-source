...
function __init__()
{
    clientfield::register( "world", "nextMap", 1, 6, "int", &next_map_changed, 0, 1 );
    ...
    setuimodelvalue( nextmapmodel, var_aed90fc4 );
...
// Size: 0xc4
function next_map_changed( localclientnum, oldval, newval, bnewent, binitialsnap, fieldname, bwastimejump )
{
    nextmapmodel = createuimodel( getglobaluimodel(), "nextMap" );
...
function function_7e781987( localclientnum )
{
    ...
    collectibles = struct::get_script_bundles_of_type( "collectible" );
    
    foreach ( item in collectibles )
    {
        if ( !isdefined( level.var_19c61a80[ item.slotsize ] ) && isdefined( item.uimodel ) && item.uimodel !== "tag_origin" )
        {
            level.var_19c61a80[ item.slotsize ] = item.uimodel;
        }
    }
    ...
    for ( n_player_index = 0; n_player_index < 4 ; n_player_index++ )
    {
        ...
        {
            item = function_1bbcb5e2( localclientnum, n_player_index, "collectible_" + n_coll );
            
            if ( isdefined( item ) )
            {
                var_e44d919a = spawnstruct();
                var_e44d919a.model = util::spawn_model( localclientnum, level.var_19c61a80[ function_48fdc243( n_coll ) ], item.origin, item.angles );
                ...
                if ( function_48fdc243( n_coll ) == 0 )
                {
                    var_e44d919a.var_56ad232d.origin += ( 0, 0, -4 );
                }
                
                level.rooms[ localclientnum ][ n_player_index ].a_coll[ n_coll ] = var_e44d919a;
            }
        }
    }
...
// Size: 0x39e
function on_collectibles_change( var_79678dca, collectiblesarray, localclientnum )
{
    ...
    assert( collectiblesarray.size == 9 );
    assert( isdefined( level.rooms ) );
    
    for ( slot = 0; slot < collectiblesarray.size ; slot++ )
    {
        if ( !( isdefined( level.rooms[ localclientnum ][ var_79678dca ].a_coll[ slot ].var_d3fca4df ) && level.rooms[ localclientnum ][ var_79678dca ].a_coll[ slot ].var_d3fca4df ) )
        {
            ...
            if ( isdefined( collectiblesarray[ slot ] ) )
            {
                var_89573eb4 = struct::get_script_bundle_list( "collectible", collectiblesarray[ slot ][ "collectiblelist" ] );
                
                if ( isdefined( var_89573eb4 ) )
                {
                    collectible = struct::get_script_bundle( "collectible", var_89573eb4[ collectiblesarray[ slot ][ "index" ] ] );
    ...
    while ( true )
    {
        ...
        if ( var_64bc1c63 )
        {
            if ( event === "clear_preview_collectible" )
            {
                ...
                {
                    collectibleslot = level.rooms[ localclientnum ][ var_a7f04717 ].a_coll[ collectibleindex ];
                    
                    if ( isdefined( collectibleslot.visible ) && isdefined( collectibleslot.var_c62c5c23 ) && collectibleslot.var_c62c5c23 && collectibleslot.visible )
                    {
                        collectibleslot.model show();
                    }
                    
                    collectibleslot.var_c62c5c23 = undefined;
                }
            }
            ...
            else if ( event === "unequip" )
            {
                ...
                for ( collectibleindex = 0; collectibleindex < level.rooms[ localclientnum ][ var_a7f04717 ].a_coll.size ; collectibleindex++ )
                {
                    ...
                    if ( isdefined( collectibleslot.visible ) && collectibleslot.visible )
                    {
                        collectibleslot.model show();
                    }
                }
            ...
            else if ( event === "focus_change" )
            {
                ...
                for ( collectibleindex = 0; collectibleindex < level.rooms[ localclientnum ][ var_a7f04717 ].a_coll.size ; collectibleindex++ )
                {
                    ...
                    if ( collectibleslot.model.model == model && collectibleindex != var_47da189b.slot )
                    {
                        collectibleslot.var_c62c5c23 = 1;
                        continue;
                    }
            ...
            for ( collectibleindex = 0; collectibleindex < level.rooms[ localclientnum ][ var_a7f04717 ].a_coll.size ; collectibleindex++ )
            {
                ...
                if ( isdefined( collectibleslot.var_c62c5c23 ) && collectibleslot.var_c62c5c23 )
                {
                    collectibleslot.model hide();
                }
            }
        }
    }
...
function function_d9b15719( localclientnum, menu_name )
{
    ...
    for ( n_coll = 0; n_coll <= 8 ; n_coll++ )
    {
        ...
        level.rooms[ localclientnum ][ var_a7f04717 ].a_coll[ n_coll ].var_9e6f06a8 = undefined;
    }
}
...
function open_choose_head_menu( localclientnum, menu_data )
{
    ...
    character_customization::update_show_helmets( localclientnum, level.liveccdata[ localclientnum ], 0 );
}
...
{
    character_customization::update_show_helmets( localclientnum, level.liveccdata[ localclientnum ], 1 );
