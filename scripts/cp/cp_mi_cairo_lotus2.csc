...
// Size: 0x4c
function function_2980c418( str_objective, b_starting )
{
    if ( b_starting )
    {
...
// Size: 0xb4
function function_441f2e2c( str_objective, b_starting )
{
    if ( b_starting || str_objective == "bridge_battle" )
    {
...
// Size: 0xa4
function function_c1e7454e( str_objective, b_starting )
{
    if ( !b_starting )
    {
        var_58e8d0ae = struct::get( "siege_anim_bridge_crowd_1" );
...
// Size: 0x9c
function function_5342496( str_objective, b_starting )
{
...
// Size: 0xcc
function industrial_zone( str_objective, b_starting )
{
    if ( !b_starting )
    {
        var_58e8d0ae = struct::get( "siege_anim_pursuit_crowd_1" );
...
// Size: 0x44
function sky_bridge1( str_objective, b_starting )
{
    if ( b_starting )
    {
        level thread scene::init( "p7_fxanim_cp_lotus_dogfight_lrg_bundle", "scriptbundlename" );
...
// Size: 0x74
function sky_bridge2( str_objective, b_starting )
{
    if ( b_starting )
    {
        level thread scene::init( "p7_fxanim_cp_lotus_dogfight_lrg_bundle", "scriptbundlename" );
...
function on_player_spawned( localclientnum )
{
    ...
    self thread lotus_util::falling_debris( localclientnum );
...
{
    a_fans = getentarray( localclientnum, "siegebot_fan", "targetname" );
    
    foreach ( e_fan in a_fans )
    {
        if ( newval )
        {
            e_fan sethighdetail( 1 );
            e_fan thread lotus_util::spinning_fan();
            continue;
        }
        
        e_fan notify( #"stop_spinning" );
        e_fan delete();
    }
}
