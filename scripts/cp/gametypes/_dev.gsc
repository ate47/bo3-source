...
/#
    ...
    function updatedevsettings()
    {
        ...
        if ( getdvarstring( "<dev string:x3d3>" ) != "<dev string:x51>" )
        {
            for ( i = 0; i < level.players.size ; i++ )
            {
                level.players[ i ] hud_message::oldnotifymessage( getdvarstring( "<dev string:x3d3>" ), getdvarstring( "<dev string:x3d3>" ), game[ "<dev string:x3e1>" ][ "<dev string:x3e7>" ] );
            }
    ...
    function watchattachmentchange()
    {
        ...
        for ( ;; )
        {
            if ( self buttonpressed( dpad_modifier_button ) )
            {
                ...
                if ( !lstick_down && self buttonpressed( "<dev string:x4e2>" ) )
                {
