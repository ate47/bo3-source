    ...
    function function_1122caac( localclientnum, oldval, newval, bnewent, binitialsnap, fieldname, bwastimejump )
    {
        ...
        switch ( newval )
        {
            ...
            case 1:
                ...
                setsaveddvar( "<dev string:x49>", 150 );
                break;
        }
    }
...
function function_d2243c73( localclientnum )
{
    ...
    while ( isdefined( self ) && isalive( self ) )
    {
        ...
        while ( self.missile_target.screenproj[ 0 ] < 128 || self.missile_target.screenproj[ 0 ] > 1152 || self.missile_target.screenproj[ 1 ] < 72 || isdefined( self ) && isdefined( self.missile_target ) && isalive( self.missile_target ) && self.missile_target.screenproj[ 2 ] > 648 )
        {
            entpos = self.missile_target gettagorigin( "tag_body" );
...
function function_c0623e13( localclientnum )
{
    ...
    while ( isdefined( self ) && isalive( self ) && self hasdobj( localclientnum ) )
    {
        ...
        else
        {
            ...
            else if ( rot_vel[ 2 ] > 5 && var_bd5365a3 != "left" )
            {
...
function function_835cb7d( localclientnum, str_type, str_fx, clear )
{
    ...
    if ( isdefined( str_fx ) )
    {
        ...
        if ( !isdefined( self.a_fx[ localclientnum ][ str_type ][ str_fx ] ) )
        {
            self.a_fx[ localclientnum ][ str_type ][ str_fx ] = [];
        }
    }
...
// Size: 0x13c
function fx_play_on_tag( localclientnum, str_type, str_fx, str_tag, b_kill_fx_with_same_type, var_de656156 )
{
...
// Size: 0x1ac
function fx_play( localclientnum, str_type, str_fx, v_pos, v_forward, v_up, b_kill_fx_with_same_type, var_de656156 )
{
    if ( !isdefined( b_kill_fx_with_same_type ) )
    {
        b_kill_fx_with_same_type = 1;
