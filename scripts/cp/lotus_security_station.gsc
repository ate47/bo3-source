...
// Size: 0x254
function function_cd269efc( str_objective, b_starting )
{
    if ( b_starting )
    {
    ...
    level.ai_khalil thread function_e8ea29f3( b_starting );
...
// Size: 0x7c
function function_e8ea29f3( b_starting )
{
    if ( !b_starting )
    {
...
// Size: 0x104
function function_a2032fbb( b_starting )
{
...
function atrium_battle( str_objective, b_starting )
{
    ...
    {
        load::function_73adcefc();
        level thread function_a2032fbb( b_starting );
...
function hack_the_system_main( str_objective, b_starting )
{
    if ( b_starting )
    {
        ...
        level.ai_hendricks setgoal( nd_start, 1 );
        level scene::init( "vent_hallway_ravens", "targetname" );
    ...
    util::init_interactive_gameobject( t_hack, &"cp_level_lotus_hack_console", &"CP_MI_CAIRO_LOTUS_HACK_SYSTEM", &function_dc25fc43 );
}
...
{
    level.ai_hendricks waittill( #"hash_a5038e77" );
...
function hack_the_system_vo()
{
    ...
    wait 1;
    level dialog::player_say( "plyr_take_it_easy_hendri_0" );
    level dialog::player_say( "plyr_this_isn_t_you_it_s_0" );
    ...
    {
        level dialog::player_say( "plyr_at_least_we_got_the_0" );
    }
    
    level flag::wait_till( "inside_security_station" );
    level.ai_hendricks dialog::say( "hend_spread_out_and_grab_0" );
...
function function_32477a75()
{
    ...
    level.ai_hendricks util::waittill_notify_or_timeout( "goal", 3 );
...
function function_986134c7()
{
    ...
    a_players_in_trig = function_bdb77f87( "trig_fxanim_vent" );
    playsoundatposition( "evt_vent_screen_shake", ( 0, 0, 0 ) );
    wait 1;
    array::run_all( a_players_in_trig, &playrumbleonentity, "cp_level_lotus_fxanim_vent" );
...
function function_bdb77f87( str_trigger )
{
    ...
    foreach ( player in level.players )
    {
        ...
        {
            if ( !isdefined( a_players_in_trig ) )
            {
                a_players_in_trig = [];
            }
            else if ( !isarray( a_players_in_trig ) )
            {
                a_players_in_trig = array( a_players_in_trig );
            }
            
            a_players_in_trig[ a_players_in_trig.size ] = player;
        }
    }
    
    return a_players_in_trig;
}
...
function function_3a2620ab()
{
    ...
    mdl_gameobject = util::init_interactive_gameobject( t_security_station_grate, &"cp_level_lotus_interact_breach", &"CP_MI_CAIRO_LOTUS_BREACH", &function_bad59a8e );
...
{
    level util::teleport_players_igc( "after_kick_grate" );
...
function prometheus_otr_main( str_objective, b_starting )
{
    ...
    level thread lotus2_sound::function_8836c025();
    var_a178d2fe = getent( "security_door_damage", "targetname" );
    var_a178d2fe hide();
    ...
    level util::teleport_players_igc( "prometheus_otr" );
    ...
    level thread lotus2_sound::function_fd00a4f2();
    var_a178d2fe stopsound( "evt_prebreach" );
    level thread security_station_breach( var_a178d2fe );
...
function function_966f4e31()
{
    ...
    level dialog::player_say( "plyr_kane_i_hope_you_r_0" );
...
{
    e_mobile_armory = lotus_util::mobile_shop_setup( "mobile_security_1_group", 1, 1, 1, "veh_t7_turret_auto_sentry" );
    level flag::wait_till( "shop_1_elevator_up" );
...
function turret_target_update()
{
    ...
    while ( true )
    {
        s_current_target = array::random( a_s_targets );
...
function function_794875cb( s_align, str_scene, a_ents )
{
    ...
    foreach ( ent in a_ents )
    {
        ...
        if ( ent.team != "axis" )
        {
            ent ai::set_ignoreme( 0 );
            ent ai::set_ignoreall( 0 );
            ent thread lotus_util::wait_to_delete( 1000 );
            continue;
        }
...
function function_80d145c5()
{
    ...
    level.ai_khalil thread function_965ed288();
    ...
    level.ai_hendricks setgoal( getnode( "hendricks_roof_goal", "targetname" ), 1 );
    level.ai_hendricks thread function_f92e06f2();
    level.ai_hendricks util::waittill_any( "goal", "failsafe_teleport" );
    ...
    e_trigger lotus_util::gather_point_wait( 0, level.heroes );
    wait 0.5;
    nd_traversal_mobile1 = getnode( "entermobile1_start", "targetname" );
    ...
    level.ai_hendricks linkto( var_47d90b65 );
    ...
    level.ai_hendricks setgoal( getnode( "end_mobile_1_roof_across_128", "targetname" ), 1 );
...
function function_c687efb()
{
    ...
    level notify( #"atrium_shop_unloaded" );
...
function function_d4538b37()
{
    ...
    level waittill( #"mobile_0_stopped" );
...
function function_4f07d8eb( a_ents )
{
    ...
    level waittill( #"vehicle_platform_mobile_shop_1_stop" );
...
{
    level flag::wait_till( "start_stair_shoot" );
    level.ai_khalil thread dialog::say( "khal_watch_your_fire_th_0" );
    level waittill( #"mobile_0_unload" );
    battlechatter::function_d9f49fba( 0 );
    level dialog::remote( "kane_there_commandeer_th_0" );
    level dialog::player_say( "plyr_copy_that_moving_0", 0.5 );
    level.ai_khalil dialog::say( "khal_here_they_come_cut_0", 0.7 );
    battlechatter::function_d9f49fba( 1 );
    trigger::wait_till( "mobile_shop_1_wait", undefined, undefined, 0 );
    level.ai_khalil dialog::say( "khal_hold_on_the_roof_i_0" );
    level flag::wait_till( "shop_1_elevator_up" );
    level.ai_khalil dialog::say( "khal_i_ll_take_us_up_giv_0" );
    level.ai_khalil dialog::say( "khal_nrc_reinforcements_a_0", 2 );
    level dialog::player_say( "plyr_you_got_a_suggestion_2", 0.25 );
    level waittill( #"hash_c4a944f0" );
    level.ai_khalil dialog::say( "khal_go_ahead_i_need_to_0", 0.5 );
    level notify( #"hash_e0d4b687" );
    level.ai_hendricks dialog::say( "hend_go_for_it_boy_scout_0", 0.25 );
    level flag::set( "mobile_shop_vo_done" );
}
...
function function_3d3111c6( a_ents )
{
    ...
    if ( isalive( ai_civ ) )
    {
        if ( isalive( ai_robot ) )
        {
            ai_civ thread function_4be7964c( ai_robot );
...
// Size: 0x54
function function_4be7964c( ai_robot )
{
    self waittill( #"death" );
    
    if ( level scene::is_active( "cin_lot_04_07_security_vign_headshot" ) )
    {
        ai_robot stopanimscripted( 0.5 );
    }
}
...
function security_station_vo()
{
    ...
    battlechatter::function_d9f49fba( 0 );
    level.ai_hendricks dialog::say( "hend_more_nrc_moving_in_0", 2 );
    level dialog::remote( "kane_they_re_locking_down_0", 1 );
    level flag::wait_till( "security_station_player_in_wall" );
    level thread namespace_66fe78fb::function_973b77f9();
    level.ai_hendricks dialog::say( "hend_moving_through_breac_0" );
    level flag::set( "hendricks_breach_line_done" );
}
...
// Size: 0x5c4
function security_station_breach( var_a178d2fe )
{
    ...
    {
        e_breacher magicgrenadetype( getweapon( "willy_pete_nd" ), struct.origin, ( 0, 0, 1 ), 0.05 );
    }
    ...
    foreach ( struct in a_s_targets )
    {
        ...
        e_breacher magicgrenadetype( getweapon( "flash_grenade" ), s_grenade.origin, v_velocity, 0.5 );
    }
    
    foreach ( struct in a_s_targets )
    {
        v_velocity = vectornormalize( struct.origin - s_grenade.origin ) * 500;
        e_breacher magicgrenadetype( getweapon( "willy_pete_nd" ), s_grenade.origin, v_velocity, 0.5 );
    }
