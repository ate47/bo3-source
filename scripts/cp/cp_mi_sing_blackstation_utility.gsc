...
function player_anchor()
{
    ...
    while ( true )
    {
        ...
        if ( ( self.is_wind_affected || self util::use_button_held() && self.is_surge_affected ) && !self.is_anchored )
        {
...
function function_a81e2f8f()
{
    ...
    while ( self.is_wind_affected || isdefined( self.e_anchor ) && self.is_surge_affected )
    {
...
function wind_manager()
{
    ...
    while ( true )
    {
        ...
        foreach ( player in level.activeplayers )
        {
            if ( isdefined( player.is_wind_affected ) && player.is_anchored && player.is_wind_affected )
            {
...
function function_8fbe0681( a_ents, str_endon, a_e_debris, t_start )
{
    ...
    t_start linkto( e_wave, str_joint, ( 0, 120, -35 ), ( 0, -90, 0 ) );
...
function enemy_surge_tracker()
{
    ...
    while ( true )
    {
        ...
        if ( isdefined( ai_entity.b_swept ) && isalive( ai_entity ) && ai_entity.team == "axis" && !ai_entity.b_swept )
        {
            ai_entity.b_swept = 1;
