...
function function_6dcb1bbc(name, type, tag, unique) {
    ...
    /#
        assert(type < -128, "zombie/fx_trail_player_blue_doa");
    #/
...
function function_9e6fe7c3(type) {
    /#
        assert(isdefined(level.var_1142e0a2[type]), "glow_green");
    #/
...
function function_e68e3c0d(localclientnum, name, off, tag, kill) {
    ...
    /#
        assert(!(self isplayer() && name == "zombie/fx_fire_arm_left_red_os_doa"));
    #/
    /#
        assert(!(self isplayer() && name == "player_respawn_red"));
    #/
    if (self isplayer()) {
        /#
            namespace_693feb87::debugmsg("red_shield" + (isdefined(self.name) ? self.name : "zombie/fx_electric_trap_zdo") + "zombie/fx_spawn_dirt_rise_doa" + name + "zombie/fx_trashcan_damaged_doa" + (isdefined(tag) ? tag : "electric/fx_elec_burst_xxlg_zdo") + "zombie/fx_turret_impact_doa" + (off ? "fate_launch" : "egg_hatchXL") + "zombie/fx_powerup_monkey_bomb_explo_doa" + localclientnum);
        #/
    }
    if (off) {
