...
function function_d3f117f9(doa, idx) {
    ...
    /#
        ...
        txt = "score" + (isdefined(idx) ? idx : "<unknown string>") + "<unknown string>" + (isdefined(doa.player) ? doa.player getentitynumber() : "<unknown string>");
...
function on_shutdown(localclientnum, ent) {
    if (isdefined(ent) && self === ent) {
        /#
            namespace_693feb87::debugmsg("<unknown string>" + (isdefined(self.name) ? self.name : self getentitynumber()));
        #/
