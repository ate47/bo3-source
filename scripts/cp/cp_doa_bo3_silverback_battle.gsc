...
{
    self doa_round::function_fe0946ac();
}
...
function function_fc48f9f3()
{
    ...
    /#
        if ( level.doa.round_number < 64 )
        {
            ...
            level.doa.zombie_health += level.doa.round_number * level.doa.zombie_health_inc;
    ...
    egg = spawn( "script_model", doa_arena::function_61d60e0b() + ( 0, 0, 48 ) );
    egg.targetname = "fido";
    ...
    foreach ( player in getplayers() )
    {
        player doa_round::function_fe0946ac( var_249f9545[ i ].origin );
...
function function_b8a39218()
{
    ...
    foreach ( player in players )
    {
        ...
        if ( !isdefined( player.doa.silverbackKills ) )
        {
            player.doa.silverbackKills = 0;
            player.doa.silverbackKillsRound = 0;
        }
        
        player.doa.silverbackKills += 1;
        player.doa.silverbackKillsRound += 1;
    }
}
...
function function_ae39e30a( einflictor, eattacker, idamage, idflags, smeansofdeath, weapon, vpoint, vdir, shitloc, vdamageorigin, timeoffset, boneindex, modelindex, surfacetype, surfacenormal )
{
    ...
    /#
        doa_utility::debugmsg( "<dev string:x28>" + self.targetname + "<dev string:x34>" + self.health - idamage );
    #/
...
function function_f5afb415( var_b84274b8, numattacks )
{
    ...
    while ( numattacks && isdefined( self.enemy ) )
    {
        ...
        trace = bullettrace( var_b84274b8.origin + ( 0, 0, 30 ), targetspot + ( 0, 0, 30 ), 0, undefined );
        ...
        {
            level thread doa_utility::debug_circle( targetspot + ( 0, 0, 20 ), 30, 3, ( 1, 0, 0 ) );
            level thread doa_utility::debug_line( targetspot + ( 0, 0, 20 ), self.origin + ( 0, 0, 20 ), 3, ( 1, 0, 0 ) );
...
function function_615e73a( var_b84274b8, numattacks )
{
    ...
    while ( numattacks )
    {
        ...
        {
            level thread doa_utility::debug_circle( targetpos + ( 0, 0, 20 ), 30, 3, ( 1, 0, 0 ) );
            var_b84274b8 thread doa_utility::debugorigin( 3, 20, ( 1, 0, 0 ) );
        }
...
function function_b830b6d7( linktag, silverback )
{
    ...
    {
        level thread doa_utility::debug_circle( trigger.origin + ( 0, 0, 20 ), 128, getdvarfloat( "scr_boss_banana_life", 5 ), ( 1, 0, 0 ) );
    }
...
function function_a753035a( egg )
{
    ...
    while ( hops )
    {
        ...
        hops--;
        egg doa_chicken::function_d63bdb9( 1 );
    }
...
function function_1d9d0ed2( chicken )
{
    ...
    while ( true )
    {
        chicken rotateto( chicken.angles + ( 0, 180, 0 ), rotate180time );
        wait 1;
    }
}
