...
/#
    ...
    function updateteamops()
    {
        ...
        if ( getdvarstring( level.teamops_dvar ) != "<dev string:x46>" )
        {
            teamops::startteamops( teamops );
            setdvar( level.teamops_dvar, "<dev string:x46>" );
        }
    }
    ...
    function updatedevsettings()
    {
        ...
        if ( getdvarstring( "<dev string:x594>" ) != "<dev string:x46>" )
        {
            for ( i = 0; i < level.players.size ; i++ )
            {
                level.players[ i ] hud_message::oldnotifymessage( getdvarstring( "<dev string:x594>" ), getdvarstring( "<dev string:x594>" ), game[ "<dev string:x5a2>" ][ "<dev string:x5a8>" ] );
            }
        ...
        if ( level.bot_overlay == 0 && getdvarint( "<dev string:x5c5>" ) == 1 )
        {
            level thread bot_overlay_think();
            level.bot_overlay = 1;
        }
        else if ( level.bot_overlay == 1 && getdvarint( "<dev string:x5c5>" ) == 0 )
        {
            level bot_overlay_stop();
            level.bot_overlay = 0;
        }
        
        if ( level.bot_threat == 0 && getdvarint( "<dev string:x5d5>" ) == 1 )
        {
            level thread bot_threat_think();
            level.bot_threat = 1;
        }
        else if ( level.bot_threat == 1 && getdvarint( "<dev string:x5d5>" ) == 0 )
        {
            level bot_threat_stop();
            level.bot_threat = 0;
        }
        
        if ( level.bot_path == 0 && getdvarint( "<dev string:x5e4>" ) == 1 )
        {
            level thread bot_path_think();
            level.bot_path = 1;
        }
        else if ( level.bot_path == 1 && getdvarint( "<dev string:x5e4>" ) == 0 )
        {
            level bot_path_stop();
            level.bot_path = 0;
        }
...
/#
    ...
    function larry_hud_init( larry )
    {
        /#
            ...
            larry.hud setshader( "<dev string:xaeb>", 135, 65 );
    ...
    {
        hasplayerweaponprev = getdvarint( "<dev string:x5d>" );
        grenadesonlyprev = getdvarint( "<dev string:x75>" );
        secondarygrenadesonlyprev = getdvarint( "<dev string:x8a>" );
        
        while ( true )
        {
            if ( hasplayerweaponprev != getdvarint( "<dev string:x5d>" ) )
            {
                hasplayerweaponprev = getdvarint( "<dev string:x5d>" );
            ...
            if ( grenadesonlyprev != getdvarint( "<dev string:x75>" ) )
            {
                grenadesonlyprev = getdvarint( "<dev string:x75>" );
            ...
            if ( secondarygrenadesonlyprev != getdvarint( "<dev string:x8a>" ) )
            {
                secondarygrenadesonlyprev = getdvarint( "<dev string:x8a>" );
    ...
    function watchattachmentchange()
    {
        ...
        for ( ;; )
        {
            if ( self buttonpressed( dpad_modifier_button ) )
            {
                ...
                if ( !lstick_down && self buttonpressed( "<dev string:xc2a>" ) )
                {
    ...
    function bot_overlay_think()
    {
        ...
        for ( ;; )
        {
            if ( getdvarint( "<dev string:xe1a>" ) != level.bot_index )
            {
                setdvar( "<dev string:xe1a>", level.bot_index );
            }
    ...
    function bot_threat_think()
    {
        ...
        for ( ;; )
        {
            if ( getdvarint( "<dev string:xe24>" ) != level.bot_index )
            {
                setdvar( "<dev string:xe24>", level.bot_index );
            }
    ...
    function bot_path_think()
    {
        ...
        for ( ;; )
        {
            if ( getdvarint( "<dev string:xe34>" ) != level.bot_index )
            {
                setdvar( "<dev string:xe34>", level.bot_index );
            }
