    ...
    function debug_show_exterior_goals()
    {
        while ( true )
        {
            if ( isdefined( level.toggle_show_exterior_goals ) && level.toggle_show_exterior_goals )
            {
                ...
                {
                    foreach ( location in zone.a_loc_types[ "<dev string:x28>" ] )
                    {
...
function zombie_damage_failsafe()
{
    ...
    for ( continue_failsafe_damage = 0; true ; continue_failsafe_damage = 0 )
    {
        wait 0.5;
...
function zombie_findnodes()
{
    ...
    if ( !isdefined( self.find_flesh_struct_string ) && isdefined( self.target ) && self.target != "" )
    {
        ...
        assert( isdefined( desired_origin ), "<dev string:x43>" + self.origin + "<dev string:x4e>" );
    ...
    assert( isdefined( node ), "<dev string:x77>" );
    ...
    if ( isdefined( self.entrance_nodes ) )
    {
        for ( i = 0; i < self.entrance_nodes.size ; i++ )
        {
            if ( self zombie_bad_path() )
            {
                ...
                println( "<dev string:xa5>" + self.origin + "<dev string:xb1>" + self.entrance_nodes[ i ].origin );
...
function zombie_death_event( zombie )
{
    ...
    if ( isactor( zombie ) )
    {
        ...
        else if ( zombie.damagemod === "MOD_GRENADE" || zombie.damagemod === "MOD_GRENADE_SPLASH" || zombie.damagemod === "MOD_EXPLOSIVE" || is_weapon_shotgun( zombie.damageweapon ) && zm_weapons::is_weapon_upgraded( zombie.damageweapon ) || zm_utility::is_placeable_mine( zombie.damageweapon ) || force_explode == 1 )
        {
            splode_dist = 180;
...
function zombie_pathing_get_breadcrumb( origin, breadcrumbs, bad_crumbs, pick_random )
{
    ...
    /#
        if ( pick_random )
        {
            zm_utility::debug_print( "<dev string:x197>" );
        }
    #/
...
function do_zombie_spawn()
{
    ...
    assert( spots.size > 0, "<dev string:x1b1>" );
    ...
    /#
        ...
        {
            debugstar( spot.origin, getdvarint( "<dev string:x1e1>" ), ( 0, 1, 0 ) );
            host_player = util::gethostplayer();
            distance = distance( spot.origin, host_player.origin );
            iprintln( "<dev string:x1fb>" + distance / 12 + "<dev string:x210>" );
        }
    #/
