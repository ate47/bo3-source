...
function init() {
    ...
    /#
        if (getdvarstring("amb_main_light") == "turbine_idle") {
            setdvar("amb_main_light", "turbine_idle");
        }
...
function init_heli_sound_values(heli_type, part_type, max_speed_vol, min_vol, max_vol, max_speed_pitch, min_pitch, max_pitch) {
    ...
    /#
        if (getdvarint("qrdrone") > 0) {
            println("veh_qrdrone_vertical" + heli_type);
            println("" + part_type);
...
/#
    ...
    function command_parser() {
        while (true) {
            command = getdvarstring("amb_main_light");
            if (command != "turbine_idle") {
                success = 1;
