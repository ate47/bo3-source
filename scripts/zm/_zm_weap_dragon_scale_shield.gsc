...
function function_459dacdd() {
    ...
    /#
        if (2 == getdvarint("riotshield_knockdown_range")) {
            near_circle_pos = view_pos + vectorscale(forward_view_angles, 2);
            circle(near_circle_pos, level.zombie_vars["zombie_knockdown_ground_impact"], (1, 0, 0), 0, 0, 100);
            line(near_circle_pos, end_pos, (0, 0, 1), 1, 0, 100);
            circle(end_pos, level.zombie_vars["zombie_knockdown_ground_impact"], (1, 0, 0), 0, 0, 100);
        }
    #/
...
function function_8e9a1613(msg, color) {
    /#
        if (!getdvarint("riotshield_knockdown_range")) {
            return;
        }
        if (!isdefined(color)) {
            color = (1, 1, 1);
