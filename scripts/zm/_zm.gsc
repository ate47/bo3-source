...
function last_stand_pistol_swap()
{
    ...
    if ( self.laststandpistol == level.default_solo_laststandpistol && ( isdefined( self._special_solo_pistol_swap ) && self._special_solo_pistol_swap || !self.hadpistol ) )
    {
...
function player_damage_override( einflictor, eattacker, idamage, idflags, smeansofdeath, weapon, vpoint, vdir, shitloc, psoffsettime )
{
    ...
    if ( isdefined( level._game_module_game_end_check ) && ( count < players.size || ![[ level._game_module_game_end_check ]]() ) )
    {
        if ( isdefined( level.force_solo_quick_revive ) && isdefined( self.lives ) && self.lives > 0 && level.force_solo_quick_revive && self hasperk( "specialty_quickrevive" ) )
        {
            self thread wait_and_revive();
        }
    ...
    if ( players.size == 1 && level flag::get( "solo_game" ) )
    {
        if ( isdefined( level.check_end_solo_game_override ) && ( isdefined( level.no_end_game_check ) && level.no_end_game_check || [[ level.check_end_solo_game_override ]]() ) )
        {
            return finaldamage;
        }
    ...
    solo_death = self.lives == 0 || players.size == 1 && level flag::get( "solo_game" ) && !self hasperk( "specialty_quickrevive" );
    non_solo_death = players.size == 1 && ( count > 1 || !level flag::get( "solo_game" ) );
...
function printhashids()
{
    /#
        ...
        {
            outputstring += "<dev string:x613>" + s_craftable.name + "<dev string:x615>" + s_craftable.hash_id + "<dev string:x165>";
            ...
            {
                outputstring += s_piece.piecename + "<dev string:x615>" + s_piece.hash_id + "<dev string:x165>";
            }
        }
