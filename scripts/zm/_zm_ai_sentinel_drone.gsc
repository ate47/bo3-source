...
function function_d600cb9a()
{
    ...
    while ( true )
    {
            ...
            if ( query_result.data.size )
            {
                ...
                foreach ( n_index, s_loc in a_s_locs )
                {
                    ...
                    if ( isdefined( var_e31f585b ) )
                    {
                        ...
                        a_ground_trace = groundtrace( var_e31f585b + ( 0, 0, 64 ), var_e31f585b + ( 0, 0, -128 ), 0, undefined );
        ...
        if ( isdefined( v_spawn_pos ) )
        {
            ...
            self sentinel_drone::sentinel_forcegoandstayinposition( 1, v_spawn_pos + ( 0, 0, 106 ) );
            self waittill( #"goal" );
    ...
    if ( isdefined( radius ) && radius > 0 )
    {
        ...
        if ( !isdefined( level.var_1ae26ca5 ) || var_199ecc3a < level.var_1ae26ca5 )
        {
            ...
            foreach ( ai_zombie in var_82aacc64 )
            {
                ...
                if ( isdefined( ai_zombie.is_elemental_zombie ) && b_ignore_elemental && ai_zombie.is_elemental_zombie )
                {
                    continue;
                }
