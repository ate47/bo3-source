...
class class_df2b89fb {
    ...
    function initialize(var_fcd89369) {
        ...
        foreach (e_ent in var_3712c214) {
            ...
            /#
                iprintlnbold("ZM_ZOD_TRAIN_USE" + namespace_8e578893::function_f7f2ffed(e_ent.origin) + "<unknown string>");
            #/
        }
        function_6f6ab7a4();
        /#
            if (!isdefined(self.var_6732a75b)) {
                /#
                    assertmsg("<unknown string>" + namespace_8e578893::function_f7f2ffed(self.var_36e768e4.origin) + "<unknown string>");
                #/
            }
        #/
    ...
    function function_a9acf9e2() {
        ...
        foreach (e_player in level.players) {
            if (function_e8e6e4b4(e_player)) {
                /#
                    if (e_player isinmovemode("ZM_ZOD_TRAIN_OFFLINE", "front_door")) {
                        continue;
                    }
                #/
    ...
    function function_4bf9e430() {
        ...
        while (true) {
            ...
            if (isdefined(nd.script_parameters)) {
                ...
                default:
                    /#
                        assertmsg("callbox" + nd.script_parameters);
                    #/
                    break;
                }
            }
    ...
    function function_11c8bd7a() {
        /#
            do {
                var_228edad9 = getdvarint("switches_enabled");
                wait(1);
            } while (!isdefined(var_228edad9) || var_228edad9 <= 0);
            while (true) {
                a_keys = getarraykeys(self.var_c2e30cf8);
                for (var_6983dc4b = 0; var_6983dc4b < self.var_c2e30cf8.size; var_6983dc4b++) {
                    ...
                    for (i = 0; i < node_set.size; i++) {
                        ...
                        if (isdefined(node.target)) {
                            node_target = getvehiclenode(node.target, "evt_train_station_bell");
                        ...
                        if (isdefined(node.target2)) {
                            var_dc338423 = getvehiclenode(node.target2, "evt_train_station_bell");
                ...
                foreach (ai in a_zombies) {
                    if (isdefined(ai.var_e0d198e4) && ai.var_e0d198e4) {
                        print3d(ai.origin + (0, 0, 100), "c1" + self.var_27e5d923.size + "cooldown", (0, 255, 0), 1);
                    }
                }
                wait(0.05);
            }
...
/#
    ...
    function function_1450b47e() {
        train = getent("<unknown string>", "evt_train_station_bell");
        if (isdefined(train)) {
