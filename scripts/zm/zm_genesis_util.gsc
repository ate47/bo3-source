...
function function_c3266652( str_targetname, n_duration, n_offset )
{
    ...
    assert( isdefined( var_bd7e807d ), "<dev string:x28>" + str_targetname + "<dev string:x3f>" );
    ...
    assert( isdefined( level.var_e17963aa[ str_targetname ] ), "<dev string:x4a>" + str_targetname + "<dev string:x60>" );
...
{
    assert( isdefined( level.zones[ str_zone ] ), "<dev string:xe8>" + str_zone + "<dev string:xee>" );
...
function function_442d17f2( v_start_pos, v_angles )
{
    ...
    v_ground_pos = bullettrace( v_start_pos, v_start_pos + ( 0, 0, -100000 ), 0, mdl_gateworm )[ "position" ];
...
function function_f3a1c8c5( var_e4342d5d, s_pos )
{
    ...
    mdl_rune = level.var_7bddd74e[ var_e4342d5d ];
    mdl_rune thread scene::play( var_b96024, mdl_rune );
    mdl_rune clientfield::set( "rq_rune_glow", 1 );
    ...
    if ( distancesquared( mdl_rune.origin, v_pos ) > 102400 )
    {
        mdl_rune.origin = v_pos;
    }
...
/#
    ...
    function watch_for_open_sesame()
    {
        ...
        var_2bbbcbec = getentarray( "<dev string:xff>", "<dev string:x10c>" );
        
        foreach ( e_power in var_2bbbcbec )
        {
            e_power.var_98e1d15 = 1;
            n_power_zone = e_power.script_int;
            level thread zm_power::turn_power_on_and_open_doors( n_power_zone );
        }
    ...
    function function_8faf1d24( v_color, str_print, n_scale, str_endon )
    {
        ...
        {
            str_endon = "<dev string:x131>";
        }
        
        if ( getdvarint( "<dev string:x143>" ) == 0 )
        {
            return;
        }
    ...
    function debug_draw_line( v_start, v_end, str_endon, v_color, str_endon2 )
    {
        ...
        while ( true )
        {
            recordline( v_start, v_end, v_color, "<dev string:x157>", self );
            wait 0.05;
        }
    }
    ...
    function function_9ff5370d( v_origin, n_radius, v_color, e_owner )
    {
        ...
        {
            recordcircle( v_origin, n_radius, v_color, "<dev string:x157>", e_owner );
            wait 0.05;
        }
    }
    ...
    function function_68a764f6( n_radius, v_color, e_owner )
    {
        ...
        while ( isdefined( self ) )
        {
            recordsphere( self.origin, n_radius, v_color, "<dev string:x157>", e_owner );
            wait 0.05;
        }
    }
    ...
    function function_ff016910( str_text, v_color, e_owner )
    {
        ...
        while ( isdefined( self ) )
        {
            record3dtext( str_text, self.origin, v_color, "<dev string:x157>", e_owner );
            wait 0.05;
        }
    }
    ...
    function function_d8db939b( str_flag, var_6e86b6bc, str_ender )
    {
        ...
        {
            print( "<dev string:x15e>" + str_flag + "<dev string:x170>" + "<dev string:x17d>" );
            
            if ( var_6e86b6bc )
            {
                iprintlnbold( "<dev string:x17f>" + str_flag + "<dev string:x170>" );
            }
        ...
        level flag::wait_till( str_flag );
        print( "<dev string:x15e>" + str_flag + "<dev string:x185>" + "<dev string:x17d>" );
        
        if ( var_6e86b6bc )
        {
            iprintlnbold( "<dev string:x17f>" + str_flag + "<dev string:x185>" );
        }
    }
    ...
    function function_c0411192()
    {
        ...
        var_3e38f0ae = struct::get_array( "<dev string:x18a>" );
        a_s_parts = struct::get_array( "<dev string:x1a0>" );
        var_3e38f0ae = arraycombine( var_3e38f0ae, a_s_parts, 0, 0 );
        a_s_parts = struct::get_array( "<dev string:x1b6>" );
        var_3e38f0ae = arraycombine( var_3e38f0ae, a_s_parts, 0, 0 );
        a_s_parts = struct::get_array( "<dev string:x1cb>" );
        var_3e38f0ae = arraycombine( var_3e38f0ae, a_s_parts, 0, 0 );
    ...
    function function_ba547024()
    {
        ...
        if ( self.targetname == "<dev string:x1cb>" )
        {
            v_color = ( 1, 1, 0 );
            n_radius = 16;
        }
        
        self thread function_ea73a995( v_color, "<dev string:x1d9>" );
    ...
    function function_b411d2a8()
    {
        ...
        {
            print( "<dev string:x202>" + self.origin + "<dev string:x21c>" );
            
            while ( !ispointonnavmesh( self.origin ) && function_4086bd17( self.origin ) )
            {
                sphere( self.origin, 16, ( 1, 0, 0 ) );
                print3d( self.origin + ( 0, 0, 16 ), self.origin + "<dev string:x232>", ( 1, 0, 0 ) );
                wait 0.05;
            }
        }
    }
    ...
    function debug_spawn_points()
    {
        ...
        a_s_points = struct::get_array( "<dev string:x247>" );
        a_s_respawn_points = [];
        ...
        var_c16df719 = struct::get_array( "<dev string:x25c>" );
    ...
    function function_f448730a()
    {
        ...
        for ( i = 1; i < 4 ; i++ )
        {
            level scene::add_scene_func( "<dev string:x26c>" + i, &function_871166cb, "<dev string:x291>" );
        }
    ...
    function function_81eb9b35()
    {
        ...
        for ( i = 1; i < 4 ; i++ )
        {
            level scene::remove_scene_func( "<dev string:x26c>" + i, &function_871166cb, "<dev string:x291>" );
        }
    ...
    {
        a_ents[ 0 ] thread function_ea73a995( ( 1, 1, 0 ), "<dev string:x296>" );
        recordent( a_ents[ 0 ] );
        ...
        if ( !ispointonnavmesh( a_ents[ 0 ].origin ) )
        {
            for ( i = 0; i < 20 ; i++ )
            {
                ...
                print3d( a_ents[ 0 ].origin + ( 0, 0, 16 ), a_ents[ 0 ].origin + "<dev string:x2bc>", ( 1, 0, 0 ) );
                wait 0.05;
            }
        }
    ...
    {
        assert( self.a_loc_types[ "<dev string:x2d2>" ].size > 0, "<dev string:xe8>" + self.volumes[ 0 ].targetname + "<dev string:x2e0>" );
    ...
    function function_2e9f38b0()
    {
        ...
        if ( self.script_noteworthy == "<dev string:x2f8>" )
        {
            return;
        }
        
        if ( !ispointonnavmesh( self.origin ) && self.script_noteworthy != "<dev string:x306>" )
        {
            print( "<dev string:x321>" + self.origin + "<dev string:x21c>" );
            
            while ( !ispointonnavmesh( self.origin ) )
            {
                sphere( self.origin, 32, ( 1, 0, 0 ) );
                print3d( self.origin + ( 0, 0, 16 ), self.origin + "<dev string:x33b>" + self.script_noteworthy + "<dev string:x343>" + self.zone_name, ( 1, 0, 0 ) );
                wait 0.05;
            }
        }
    }
    ...
    {
        if ( self.script_noteworthy == "<dev string:x306>" )
        {
            if ( !isdefined( self.var_640b69b2 ) )
            {
                self.var_640b69b2 = util::spawn_model( "<dev string:x346>", self.origin, self.angles );
            }
            
            self.var_640b69b2 scene::play( "<dev string:x26c>" + randomintrange( 1, 3 ) );
    ...
    function function_835d6248()
    {
        ...
        if ( self.script_noteworthy == "<dev string:x306>" )
        {
            while ( true )
            {
                ...
                {
                    self.var_640b69b2 = util::spawn_model( "<dev string:x346>", self.origin, self.angles );
                    self.var_640b69b2 thread function_ea73a995( undefined, "<dev string:x296>" );
                }
    ...
    function function_3e617e39()
    {
        if ( self.script_noteworthy == "<dev string:x306>" )
        {
            if ( isdefined( self.var_640b69b2 ) )
            {
                for ( i = 1; i < 4 ; i++ )
                {
                    self.var_640b69b2 scene::stop( "<dev string:x26c>" + i );
                }
    ...
    function function_7129ea51()
    {
        ...
        if ( !ispointonnavmesh( self.origin ) )
        {
            print( "<dev string:x351>" + self.origin + "<dev string:x21c>" );
            
            while ( !ispointonnavmesh( self.origin ) )
            {
                sphere( self.origin, 16, ( 1, 0, 0 ) );
                print3d( self.origin + ( 0, 0, 16 ), self.origin + "<dev string:x36d>" + self.script_noteworthy, ( 1, 0, 0 ) );
                wait 0.05;
            }
        }
    }
    ...
    function function_1953a761()
    {
        ...
        a_s_points = struct::get_array( "<dev string:x384>" );
    ...
    function function_ed69d7f4()
    {
        ...
        {
            print( "<dev string:x392>" + self.origin + "<dev string:x21c>" );
            
            while ( true )
            {
                sphere( self.origin, 61.88, ( 1, 1, 0 ) );
                print3d( self.origin + ( 0, 0, 16 ), self.origin + "<dev string:x3aa>", ( 1, 1, 0 ) );
                wait 0.05;
            }
        }
    }
    ...
    function function_4086bd17( v_point )
    {
        ...
        if ( distance2dsquared( a_trace[ "<dev string:x3bf>" ], v_point ) > 36 )
        {
            return 1;
        }
    ...
    function function_90c620d8()
    {
        ...
        var_9f26317f = getentarray( "<dev string:x3c8>", "<dev string:x10c>" );
    ...
    function function_5e8cafb9()
    {
        ...
        foreach ( t_door in self )
        {
            ...
            {
                level thread function_d8db939b( t_door.script_flag, 1, "<dev string:x3d4>" );
            }
        }
    }
    ...
    {
        var_3ccf13b = [];
        a_e_targets = getentarray( self.target, "<dev string:x10c>" );
        
        foreach ( e_target in a_e_targets )
        {
            if ( e_target.classname == "<dev string:x3f3>" || e_target.script_string === "<dev string:x400>" )
            {
    ...
    function function_bc81bb3b()
    {
        ...
        if ( !isdefined( self.script_flag ) )
        {
            if ( isdefined( self.script_noteworthy ) )
            {
                while ( true )
                {
                    print3d( self.origin + ( 0, 0, 64 ), "<dev string:x409>" + self.script_noteworthy, ( 0, 0, 1 ), 1, 0.5 );
                    wait 0.05;
                }
            }
        ...
        while ( true )
        {
            ...
            print3d( self.origin + ( 0, 0, 64 ), "<dev string:x416>" + self.script_flag, v_color, 1, 0.5 );
            ...
            if ( isdefined( level.zone_flags[ self.script_flag ] ) )
            {
                ...
                foreach ( j, str_flag in level.zone_flags[ self.script_flag ] )
                {
                    ...
                    print3d( self.origin + ( 0, 0, 64 + var_3b72f06c[ j ] ), "<dev string:x424>" + str_flag, var_1adee14a, 1, 0.5 );
                }
            }
    ...
    function function_b3db2cea()
    {
        ...
        {
            print( "<dev string:x432>" + self.origin + "<dev string:x21c>" );
            
            while ( !ispointonnavmesh( self.origin ) )
            {
                sphere( self.origin, 96, ( 1, 1, 0 ), 0.5 );
                print3d( self.origin + ( 0, 0, 16 ), "<dev string:x448>", ( 1, 1, 0 ) );
                wait 0.05;
            }
        }
    }
    ...
    function devgui()
    {
        adddebugcommand( "<dev string:x454>" );
        adddebugcommand( "<dev string:x4b8>" );
        adddebugcommand( "<dev string:x521>" );
        adddebugcommand( "<dev string:x57c>" );
        adddebugcommand( "<dev string:x5e1>" );
        adddebugcommand( "<dev string:x636>" );
        adddebugcommand( "<dev string:x695>" );
        adddebugcommand( "<dev string:x706>" );
        adddebugcommand( "<dev string:x767>" );
        adddebugcommand( "<dev string:x7d2>" );
        adddebugcommand( "<dev string:x81b>" );
        adddebugcommand( "<dev string:x867>" );
        adddebugcommand( "<dev string:x8b1>" );
        adddebugcommand( "<dev string:x8fa>" );
        adddebugcommand( "<dev string:x948>" );
        adddebugcommand( "<dev string:x99e>" );
        adddebugcommand( "<dev string:x9f0>" );
        adddebugcommand( "<dev string:xa46>" );
        adddebugcommand( "<dev string:xa9a>" );
        level thread setup_devgui_func( "<dev string:xaec>", "<dev string:xb0b>", 1, &zm_genesis_minor_ee::function_e1963311 );
        zm_devgui::add_custom_devgui_callback( &zm_genesis_devgui );
    }
    ...
    // Size: 0x53e, Type: dev
    function zm_genesis_devgui( cmd )
    {
        cmd_strings = strtok( cmd, "<dev string:xb16>" );
        
        switch ( cmd_strings[ 0 ] )
        {
            case "<dev string:xb18>":
                var_9f26317f = getentarray( "<dev string:x3c8>", "<dev string:x10c>" );
                ...
                break;
            case "<dev string:x3d4>":
                level notify( #"hash_27118146" );
                break;
            case "<dev string:xb32>":
                function_712a86f4( 1 );
                break;
            case "<dev string:xb42>":
                function_712a86f4( 2 );
                break;
            case "<dev string:xb52>":
                function_712a86f4( 3 );
                break;
            case "<dev string:xb62>":
                function_712a86f4( 4 );
                break;
            case "<dev string:xb72>":
                function_f448730a();
                break;
            case "<dev string:xb8a>":
                function_81eb9b35();
                break;
            case "<dev string:xba7>":
                function_6c518807();
                break;
            case "<dev string:x1d9>":
                level notify( #"hash_6fd18b70" );
                break;
            case "<dev string:xbcb>":
                level.activeplayers[ 0 ] function_4667bb64();
                break;
            case "<dev string:xbd9>":
                level.activeplayers[ 0 ] function_4530c3b2();
                break;
            case "<dev string:xbe8>":
                level.activeplayers[ 0 ] function_c78305e0();
                break;
            case "<dev string:xbf5>":
                level.activeplayers[ 0 ] function_d5c8a6c2();
                break;
            case "<dev string:xc02>":
            ...
            case "<dev string:xc16>":
                ...
                break;
            case "<dev string:xc2f>":
                level flag::set( "<dev string:xc4c>" );
                break;
            case "<dev string:xc72>":
                ...
                break;
            case "<dev string:xc83>":
                level thread function_76c3e6c4();
                break;
            case "<dev string:xc95>":
                foreach ( e_player in level.activeplayers )
                {
    ...
    // Size: 0x10a, Type: dev
    function function_712a86f4( n_power_zone )
    {
        var_2bbbcbec = getentarray( "<dev string:xff>", "<dev string:x10c>" );
        
        foreach ( e_power in var_2bbbcbec )
        {
            if ( e_power.script_int == n_power_zone )
            {
                e_power.var_98e1d15 = 1;
                n_power_zone = e_power.script_int;
                level thread zm_power::turn_power_on_and_open_doors( n_power_zone );
        ...
        while ( true )
        {
            for ( i = 0; i < level.activeplayers.size ; i++ )
            {
                ...
                if ( isdefined( level.activeplayers[ i ] ) )
                {
                    ...
                    {
                        var_f1ef8e43[ level.activeplayers[ i ].name ] = function_a1bfa962( level.activeplayers[ i ], level.activeplayers[ i ].name + "<dev string:xca3>", 2, n_ypos );
                        var_2f06ca53++;
                    }
    ...
    function function_76c3e6c4()
    {
        foreach ( e_player in level.activeplayers )
        {
            ...
            e_player setperk( "<dev string:xcb3>" );
