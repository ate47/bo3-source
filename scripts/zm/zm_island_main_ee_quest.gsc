...
function function_df4d1d4()
{
    ...
    while ( true )
    {
        ...
        if ( !self.has_gasmask && zm_utility::is_player_valid( self ) )
        {
...
function function_75e5527f()
{
    ...
    while ( true )
    {
        ...
        {
            if ( player istouching( t_gas ) && !player.has_gasmask && zm_utility::is_player_valid( player ) )
            {
...
function function_6cc2e374()
{
    ...
    while ( true )
    {
        ...
        if ( level flag::get( "elevator_part_gear1_found" ) && !level flag::get( "elevator_part_gear1_placed" ) )
        {
            ...
            var_66bf6df.mdl_teleport = util::spawn_model( "p7_zm_isl_elevator_gears_wheel", var_66bf6df gettagorigin( "wheel_01_jnt" ) + ( 1, 2, 0 ), var_66bf6df gettagangles( "wheel_01_jnt" ) );
        }
        ...
        if ( level flag::get( "elevator_part_gear3_found" ) && !level flag::get( "elevator_part_gear3_placed" ) )
        {
            ...
            var_f022cb65 playsound( "zmb_item_pickup" );
            var_66bf6df.mdl_plane = util::spawn_model( "p7_zm_isl_elevator_gears_wheel_small", var_66bf6df gettagorigin( "wheel_03_jnt" ) + ( 0, 2, 0 ), var_66bf6df gettagangles( "wheel_03_jnt" ) );
        }
...
function elevator_gears()
{
    ...
    while ( true )
    {
        ...
        while ( a_e_elevator[ 0 ].is_moving )
        {
            ...
            self.mdl_teleport rotateroll( n_rot, 1 );
            self.mdl_dock rotateroll( n_rot, 1 );
            self.mdl_plane rotateroll( n_rot * -1, 1 );
            wait 0.9;
        }
        
        self.mdl_teleport rotateroll( n_rot * 0.01, 0.1 );
...
function elevator_init()
{
    ...
    n_height = 128;
    n_length = 64;
    a_s_org = struct::get_array( "s_elevator_trigger", "targetname" );
    
    foreach ( s_org in a_s_org )
    {
    ...
    a_s_org = struct::get_array( "s_elevator_call_trigger", "targetname" );
...
function function_7b8e6e93()
{
    ...
    while ( true )
    {
        ...
        foreach ( player in level.players )
        {
            if ( player.origin[ 2 ] > self.origin[ 2 ] + 60 || player.in_elevator === 1 && player.origin[ 2 ] < self.origin[ 2 ] - 20 )
            {
...
/#
    ...
    function function_efbc0e1( cmd )
    {
        switch ( cmd )
        {
            case "<dev string:x2a2>":
                level flag::set( "<dev string:x2b3>" );
                return 1;
            default:
                level thread function_85b23415();
                level flag::set( "<dev string:x2db>" );
                level flag::set( "<dev string:x2f4>" );
                level flag::set( "<dev string:x300>" );
                level flag::set( "<dev string:x309>" );
                return 1;
            case "<dev string:x31f>":
                foreach ( player in level.players )
                {
                    w_current = player getcurrentweapon();
                ...
                return 1;
            case "<dev string:x330>":
                elevator_init();
                return 1;
            case "<dev string:x340>":
                level flag::set( "<dev string:x34f>" );
                level flag::set( "<dev string:x369>" );
                level flag::set( "<dev string:x383>" );
                return 1;
            case "<dev string:x39d>":
                clientfield::set( "<dev string:x3ac>", 1 );
                level scene::init( "<dev string:x3c1>" );
                
                while ( !level scene::is_ready( "<dev string:x3c1>" ) )
                {
                    wait 0.05;
                }
                
                wait 5;
                level flag::set( "<dev string:x3d9>" );
                return 1;
            case "<dev string:x3f2>":
