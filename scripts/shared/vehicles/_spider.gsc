...
function should_switch_to_melee( from_state, to_state, connection )
{
    ...
    if ( distance2dsquared( self.origin, self.enemy.origin ) < self.settings.meleedist * self.settings.meleedist && ( self.switch_to_melee === 1 || abs( self.origin[ 2 ] - self.enemy.origin[ 2 ] ) < self.settings.meleedist ) )
    {
        return true;
    }
            ...
            if ( distancesquared( self.origin, targetpos ) > 1000 * 1000 && self isposinclaimedlocation( targetpos ) )
            {
                ...
                foreach ( point in queryresult.data )
                {
                    /#
                        ...
                        point._scoredebug[ "<dev string:x3e>" ] = mapfloat( 0, 200, 0, -200, distance( point.origin, queryresult.origin ) );
                    #/
                    ...
                    /#
                        ...
                        point._scoredebug[ "<dev string:x8b>" ] = mapfloat( 50, 200, 0, -200, abs( point.origin[ 2 ] - queryresult.origin[ 2 ] ) );
                    #/
