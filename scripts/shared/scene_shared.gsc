...
// Namespace scene
// Method(s) 5 Total 70
class cawarenessscene : cscene, cscriptbundlebase
{
    ...
    // Size: 0x1c4
    function play( str_awareness_state )
    {
        if ( !isdefined( str_awareness_state ) )
        {
            str_awareness_state = "low_alert";
        }
...
class csceneobject : cscriptbundleobjectbase
{
    ...
    {
        if ( isdefined( self._s.deletewhenfinished ) && isdefined( [[ self._o_bundle ]]->is_skipping_scene() ) && !( isdefined( self._s.player ) && self._s.player ) && !( isdefined( self._s.sharedigc ) && self._s.sharedigc ) && !( isdefined( self._s.keepwhileskipping ) && self._s.keepwhileskipping ) && [[ self._o_bundle ]]->is_skipping_scene() && self._s.deletewhenfinished )
        {
    ...
    {
        b_disable_throttle = isdefined( self._o_bundle._s.dontthrottle ) && ( self._o_bundle._s scene::is_igc() || self._o_bundle._s.dontthrottle );
    ...
    function _play_anim( animation, n_delay_min, n_delay_max, n_rate, n_blend, n_time )
    {
        ...
        do_reach = !( isdefined( self._o_bundle._testing ) && self._o_bundle._testing ) || isdefined( self._s.doreach ) && self._s.doreach && n_time == 0 && getdvarint( "scene_test_with_reach", 0 );
    ...
    function _cleanup()
    {
        ...
        if ( isdefined( self._o_bundle.scene_stopping ) && isdefined( self._o_bundle ) && self._o_bundle.scene_stopping )
        {
    ...
    function set_objective()
    {
        if ( !isdefined( self._e.script_objective ) )
        {
            ...
            if ( isdefined( self._o_bundle._s.script_objective ) )
            {
        ...
        if ( isdefined( self._str_state ) )
        {
            ...
            if ( isdefined( self._e ) )
            {
                if ( !is_player() )
                {
                    ...
                    else if ( isdefined( self._s.diewhenfinished ) && is_alive() && self._s.diewhenfinished && !b_canceled )
                    {
                        self._e.skipdeath = 1;
                        self._e.allowdeath = 1;
    ...
    function _prepare_player( player )
    {
        ...
        if ( isdefined( self._s.firstweaponraise ) && self._s.firstweaponraise )
        {
        }
    ...
    function _spawn( e_spawner, b_hide, b_set_ready_when_spawned )
    {
        ...
        b_skip = self._s.type === "player" && ( b_skip || issubstr( self._o_bundle._str_mode, "noplayers" ) );
        ...
        if ( !b_skip )
        {
            ...
            else if ( !isdefined( self._e ) || isspawner( self._e ) )
            {
                ...
                if ( !isspawner( self._e ) )
                {
                    ...
                    if ( !( isdefined( self._s.nospawn ) && self._s.nospawn ) && !self._b_spawnonce_used || ( !isdefined( e ) || isspawner( e ) ) && self._o_bundle._testing )
                    {
    ...
    function initialize( b_force_first_frame )
    {
        ...
        if ( has_init_state() || b_force_first_frame )
        {
            ...
            if ( isdefined( self._s.spawnoninit ) && !( isdefined( self._s.sharedigc ) && self._s.sharedigc ) && !( isdefined( self._s.player ) && self._s.player ) && self._s.spawnoninit || b_force_first_frame )
            {
...
class cscene : cscriptbundlebase
{
    ...
    function private trigger_scene_sequence_started( scene_object, entity )
    {
        ...
        if ( isplayer( entity ) )
        {
            if ( !( isdefined( self._s.disablesceneskipping ) && self._s.disablesceneskipping ) && !is_skipping_scene() )
            {
                ...
                {
                    if ( isdefined( scene_object._s.initanim ) || self._str_state != "init" || isdefined( scene_object._s.initanimloop ) )
                    {
                        init_scene_sequence_started( 1 );
                    }
                }
    ...
    {
        if ( isdefined( self._s.disablesceneskipping ) && isdefined( b_sequence ) && b_sequence && self._s.disablesceneskipping )
        {
            /#
                if ( getdvarint( "<dev string:x125>" ) > 0 )
                {
...
function skip_scene( scene_name, b_sequence, b_player_scene, b_check_linked_scene )
{
    ...
    if ( isdefined( b_player_scene ) && !( isdefined( b_sequence ) && b_sequence ) && b_player_scene )
    {
...
function _skip_scene( skipped_scene_name, b_sequence, b_player_scene, b_check_linked_scene )
{
    ...
    if ( isdefined( self.scenes ) )
    {
        foreach ( o_scene in arraycopy( self.scenes ) )
        {
            ...
            if ( isdefined( b_player_scene ) && !( isdefined( b_sequence ) && b_sequence ) && b_player_scene && !( isdefined( b_check_linked_scene ) && b_check_linked_scene ) )
            {
            ...
            if ( !( isdefined( b_sequence ) && b_sequence ) )
            {
                ...
                if ( isdefined( o_scene._s.disablesceneskipping ) && ( !b_linked_scene || isdefined( b_check_linked_scene ) && b_check_linked_scene && o_scene._s.disablesceneskipping ) )
                {
                    continue;
                }
                ...
                if ( b_linked_scene && ( !isdefined( skipped_scene_name ) || str_scene_name == skipped_scene_name || !( isdefined( o_scene._s.disablesceneskipping ) && o_scene._s.disablesceneskipping ) ) )
                {
                    if ( isdefined( b_player_scene ) && ( !isdefined( skipped_scene_name ) || str_scene_name == skipped_scene_name ) && b_player_scene && !( isdefined( b_check_linked_scene ) && b_check_linked_scene ) && !b_linked_scene )
                    {
                        b_shared_scene = 1;
