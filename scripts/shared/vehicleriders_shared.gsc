...
function get_in( vh, str_pos, b_teleport )
{
    ...
    if ( !isdefined( vh.ignore_seat_check ) || !vh.ignore_seat_check )
    {
        ...
        if ( seat_index <= 4 )
        {
            ...
            assert( seat_available, "<dev string:x4f>" );
...
function unload( str_group, str_mode, remove_rider_before_unloading, remove_riders_wait_time )
{
    ...
    assert( isdefined( level.vehiclerider_groups[ str_group ] ), str_group + "<dev string:x104>" );
...
function exit_ground()
{
    ...
    assert( isstring( self.rider_info.exitgroundanim ), "<dev string:x122>" + self.rider_info.position + "<dev string:x13a>" );
...
function exit_low()
{
    ...
    assert( isdefined( self.rider_info.exitlowanim ), "<dev string:x122>" + self.rider_info.position + "<dev string:x13a>" );
...
function private forward_euler_integration( e_move, v_target_landing, n_initial_speed )
{
    ...
    while ( !landed )
    {
        ...
        velocity += gravity * integrationstep;
        position += velocity * integrationstep;
...
function exit_variable()
{
    ...
    assert( isdefined( self.rider_info.exithighanim ), "<dev string:x122>" + self.rider_info.position + "<dev string:x13a>" );
    ...
    if ( isdefined( self.dropundervehicleoriginoverride ) && ( isdefined( self.rider_info.dropundervehicleorigin ) && self.rider_info.dropundervehicleorigin || self.dropundervehicleoriginoverride ) )
    {
...
function private _init_rider( vh, str_pos )
{
    ...
    assert( isdefined( self.rider_info ) || isdefined( str_pos ), "<dev string:x237>" );
